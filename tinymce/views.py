# coding=utf-8
# Автор: Андрея Гладилина [http://www.computerica.ru/]
# Исправления: proft [proft.com.ua] -> ShowDir()
import shutil
...
STORAGE_ROOT = os.path.join(settings.MEDIA_ROOT, settings.TINYMCE_IM_DIR)
STORAGE_URL = settings.MEDIA_URL + settings.TINYMCE_IM_DIR
...
class Thumbs:
    def __init__(self, path):
        self.top = path.strip('/') + '/'
        if self.top != '/':
            self.dbfile = os.path.join(STORAGE_ROOT, self.top, '.thumbs', '.db')
        else:
            self.dbfile = os.path.join(STORAGE_ROOT, '.thumbs', '.db')
...
def ShowDir(top):
    ...
    furl = STORAGE_URL + top
    
    files = Thumbs(top).load()
    
    #print "listing files from %s" % (STORAGE_ROOT + top)
    ret = ""
    for file in os.listdir(os.path.join(STORAGE_ROOT, top)):
        ext = os.path.splitext(file)
        if ext[1] not in ('.jpg', '.png', '.gif', '.JPG', '.PNG', '.GIF'):
            continue
        #import pdb;pdb.set_trace()
        if os.path.isfile(os.path.join(STORAGE_ROOT, top, file)):
            if files.has_key(file):
                info = files[file]
                ext = info['ext'].upper()
                linkto = furl + info['link']
                fsize = info['size']
                fdate = info['date']
                fwidth = info['width']
                fheight = info['height']
...
def UploadFile(request):
    ...
    if (len(request.FILES)):
        for file in request.FILES.items():
            ...
            filename = name + '.' + ext
            filelink = filename
            filepath = os.path.join(STORAGE_ROOT, top, filename)
